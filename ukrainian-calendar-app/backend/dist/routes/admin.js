...
router.post('/events/:id/images', upload.single('image'), async (req, res) => {
    try {
        ...
        const optimizedPath = path_1.default.join(req.file.destination, optimizedFileName);
...
router.delete('/events/:eventId/images/:imageId', async (req, res) => {
    try {
        ...
        const image = await database_1.default.query('SELECT image_url FROM event_images WHERE id = $1', [imageId]);
        ...
        // Видаляємо файл
        const imagePath = path_1.default.join(__dirname, '../../', image.rows[0].image_url);
        try {
        ...
        // Видаляємо з БД
        await database_1.default.query('DELETE FROM event_images WHERE id = $1', [imageId]);
        res.json({
            success: true,
