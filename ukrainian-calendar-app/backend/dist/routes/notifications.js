...
router.post('/register', async (req, res) => {
    try {
        ...
        const existing = await database_1.default.query('SELECT id FROM device_tokens WHERE fcm_token = $1', [fcmToken]);
        let deviceTokenId;
        if (existing.rows.length > 0) {
            // Оновлюємо існуючий токен
            const update = await database_1.default.query(`
...
router.put('/preferences', async (req, res) => {
    try {
        ...
        const device = await database_1.default.query('SELECT id FROM device_tokens WHERE fcm_token = $1', [fcmToken]);
